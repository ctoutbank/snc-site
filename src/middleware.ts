...
//
// Por que isso existe:
// - snc.consolle.one (snc-site) tem páginas LOCAIS (home, artigos, lgpd, /relatorio/snc/exemplo)
//   cujos chunks JS estão em SEU build (snc-site/.next/static).
// - snc.consolle.one TAMBÉM proxia /auth, /portal, /relatorio/snc/[id] do backend
//   (outbank-one), cujo HTML referencia chunks /_next/static/* que vivem no outbank-one.
...
// - Referer = /auth, /portal, /relatorio/snc/<id-numerico>  →  redireciona para outbank.cloud
// - Referer = qualquer outra coisa (ou ausente)  →  serve local
//
// Histórico: comentário "assetPrefix removido" em outbank-one/next.config.js confirma
// que a alternativa (assetPrefix global no outbank-one) já foi tentada e gerou outro bug.

const PORTAL = process.env.PORTAL_PROXY_URL || "https://outbank.cloud";

// Regex de paths que são proxiados do backend
const PROXIED_PATHS = /\/(auth|portal)(\/|$)|\/relatorio\/snc\/\d+(\/|$)/;
