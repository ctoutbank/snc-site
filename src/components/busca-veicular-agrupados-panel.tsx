...
interface VeicularAgrupadosResult {
...
function exibirPlaca(placa: string): string {
...
export function BuscaVeicularAgrupadosPanel() {
  const [placa, setPlaca] = useState("");
  const [loading, setLoading] = useState(false);
  const [erro, setErro] = useState<string | null>(null);
  const [resultado, setResultado] = useState<VeicularAgrupadosResult | null>(null);
  const { historico, salvar, limpar } = useHistoricoConsultas("veicular-agrupados");
  ...
  const handleBuscar = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/apibrasil/veicular-agrupados?placa=${clean}`);
      const data = await res.json();
      if (!res.ok) throw new Error(data.error ?? "Erro na consulta.");
      setResultado(data as VeicularAgrupadosResult);
  ...
  const handleExemplo = useCallback(() => {
    ...
    const mockData: VeicularAgrupadosResult = {
      veiculo: {
        placa: "SDD4545", marca_modelo: "VW/FOX 1.0 GII",
        ano_fabricacao: "2012", ano_modelo: "2013", cor: "VERMELHA",
        combustivel: "ALCOOL/GASOLINA", chassi: "9BFZF55A0E8008912",
        renavam: "00456789012", motor: "CCC178906",
        municipio: "CURITIBA", uf: "PR", especie: "PASSAGEIRO",
        tipo: "AUTOMOVEL", carroceria: "HATCH", potencia: "76CV",
        cilindrada: "999", capacidade_passageiros: "5",
        procedencia: "NACIONAL", situacao: "CIRCULACAO",
      },
      fipe: {
        codigo_fipe: "005340-6", marca: "VW - VOLKSWAGEN",
        modelo: "FOX 1.0 MI TOTAL FLEX 8V 5P", ano_modelo: "2013 Gasolina",
        combustivel: "GASOLINA", valor: "R$ 32.457,00",
        referencia: "junho de 2025",
      },
      proprietario: {
        nome: "JOÃO DA SILVA", documento: "***456789**",
        tipo_documento: "CPF", municipio: "CURITIBA", uf: "PR",
  ...
  const vei = (resultado?.veiculo ?? {}) as Record<string, string>;
  const fipe = (resultado?.fipe ?? {}) as Record<string, string>;
  const prop = (resultado?.proprietario ?? {}) as Record<string, string>;
  ...
  return (
    <div>
      ...
      {/* ── Resultados ── */}
      {resultado && (
        <div id="agrupados-resultado" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {/* Header */}
          <div style={{
            ...
            <div style={{ display: "flex", flexDirection: "column", flex: 1, minWidth: 0 }}>
              <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: COR_ACCENT, letterSpacing: "0.14em", fontWeight: 700 }}>VEICULAR AGRUPADOS · {exibirPlaca(placa)}</span>
              <span style={{ fontSize: 22, color: "#fff", fontWeight: 700, fontFamily: "'Libre Caslon Text', serif", marginTop: 6, letterSpacing: "0.02em" }}>
                {vei.marca_modelo ? String(vei.marca_modelo).toUpperCase().replace('/', ' - ') : 'Veículo'}
              </span>
              <span style={{
                fontSize: 11, fontFamily: "'JetBrains Mono', monospace", marginTop: 6, fontWeight: 700, letterSpacing: "0.05em",
                color: "#2ba84a",
              }}>
                CONSULTA AGRUPADA COM SUCESSO
              </span>
            </div>
            <button
              ...
              onMouseEnter={(e) => { if (isFromHistory) { e.currentTarget.style.background = "rgba(255,255,255,0.08)"; e.currentTarget.style.borderColor = COR_ACCENT; e.currentTarget.style.color = "#fff"; } else { e.currentTarget.style.background = "#b8913c"; } }}
              onMouseLeave={(e) => { if (isFromHistory) { e.currentTarget.style.background = "rgba(255,255,255,0.04)"; e.currentTarget.style.borderColor = "rgba(255,255,255,0.12)"; e.currentTarget.style.color = "#cfd6df"; } else { e.currentTarget.style.background = COR_ACCENT; } }}
            >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {/* ── Ficha Cadastral ── */}
            <Bloco titulo="Ficha Cadastral do Veículo" badge="AGREGADOS">
              <div className="snc-data-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 32px" }}>
                <div>
                  <DataRow label="Marca / Modelo" value={vei.marca_modelo ? String(vei.marca_modelo).toUpperCase().replace('/', ' - ') : "—"} />
                  <DataRow label="Ano Fabricação" value={String(vei.ano_fabricacao ?? "—")} mono />
                  <DataRow label="Ano Modelo" value={String(vei.ano_modelo ?? "—")} mono />
                  <DataRow label="Cor" value={String(vei.cor ?? "—")} />
                  <DataRow label="Combustível" value={String(vei.combustivel ?? "—")} />
                  <DataRow label="Placa" value={vei.placa ? exibirPlaca(String(vei.placa)) : "—"} mono />
                  <DataRow label="Município / UF" value={vei.municipio && vei.uf ? `${vei.municipio} / ${vei.uf}` : "—"} />
                </div>
                <div>
                  <DataRow label="Espécie" value={String(vei.especie ?? "—")} />
                  <DataRow label="Tipo" value={String(vei.tipo ?? "—")} />
                  <DataRow label="Carroceria" value={String(vei.carroceria ?? "—")} />
                  <DataRow label="Potência" value={String(vei.potencia ?? "—")} />
                  <DataRow label="Cilindrada" value={String(vei.cilindrada ?? "—")} mono />
                  <DataRow label="Cap. Passageiros" value={String(vei.capacidade_passageiros ?? "—")} mono />
                  <DataRow label="Procedência" value={String(vei.procedencia ?? "—")} />
                </div>
              </div>
            </Bloco>
            ...
            <Bloco titulo="Agregados & Números de Chassi" badge="ESTRUTURAL">
              <div className="snc-data-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 32px" }}>
                <div>
                  <DataRow label="Chassi" value={String(vei.chassi ?? "—")} mono />
                  <DataRow label="RENAVAM" value={String(vei.renavam ?? "—")} mono />
                </div>
                <div>
                  <DataRow label="Número do Motor" value={String(vei.motor ?? "—")} mono />
                  <DataRow label="Situação" value={String(vei.situacao ?? "—")} />
                </div>
              </div>
            </Bloco>

            {/* ── Proprietário Atual ── */}
            <Bloco titulo="Proprietário Atual" badge="PROPRIETÁRIO">
              <div className="snc-data-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 32px" }}>
                <div>
                  <DataRow label="Nome" value={String(prop.nome ?? "—")} />
                  <DataRow label="Documento" value={String(prop.documento ?? "—")} mono />
                  <DataRow label="Tipo Documento" value={String(prop.tipo_documento ?? "—")} />
                </div>
                <div>
                  <DataRow label="Município / UF" value={prop.municipio && prop.uf ? `${prop.municipio} / ${prop.uf}` : "—"} />
                  <DataRow label="Data Atualização" value={String(prop.data_atualizacao ?? "—")} mono />
                </div>
              </div>
            </Bloco>

            {/* ── Tabela FIPE ── */}
            <Bloco titulo="Tabela FIPE" badge="FIPE">
              <div className="snc-data-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 32px" }}>
                <div>
                  <DataRow label="Código FIPE" value={String(fipe.codigo_fipe ?? "—")} mono amber />
                  <DataRow label="Marca" value={String(fipe.marca ?? "—")} />
                  <DataRow label="Modelo" value={String(fipe.modelo ?? "—")} />
                </div>
                <div>
                  <DataRow label="Ano Modelo" value={String(fipe.ano_modelo ?? "—")} />
                  <DataRow label="Valor FIPE" value={String(fipe.valor ?? "—")} amber />
                  <DataRow label="Referência" value={String(fipe.referencia ?? "—")} />
                </div>
              </div>
            </Bloco>
          </div>
      ...
      <div className={`historico-wrapper ${showHistoryMobile ? "show-mobile" : ""}`}>
        <HistoricoConsultas
          ...
          onCarregar={(dados, p) => {
            setPlaca(p);
            setResultado(dados as unknown as VeicularAgrupadosResult);
            setErro(null);
          ...
          onLimpar={limpar}
          corAccent="#D4A843"
          scrollTargetId="agrupados-resultado"
        />
      </div>
      ...
      <style>{`
        ...
        @media (max-width: 768px) {
          .search-bar-wrapper { padding: 20px 16px !important; }
          .search-bar-container { flex-direction: column; }
          #agrupados-placa-input { font-size: 18px !important; padding: 16px 16px 16px 72px !important; letter-spacing: 0.12em !important; }
          #agrupados-consultar-btn { width: 100% !important; justify-content: center !important; padding: 16px !important; }
          .snc-bloco { padding: 18px 16px !important; }
          .snc-data-grid { grid-template-columns: 1fr !important; gap: 0 !important; }
          .snc-data-row { display: flex !important; flex-direction: column !important; align-items: flex-start !important; gap: 4px !important; padding: 10px 12px !important; }
