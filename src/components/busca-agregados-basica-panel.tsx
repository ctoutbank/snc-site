...
export function BuscaAgregadosBasicaPanel() {
  const [placa, setPlaca] = useState("");
  const [loading, setLoading] = useState(false);
  const [erro, setErro] = useState<string | null>(null);
  const [resultado, setResultado] = useState<VeiculoResult | null>(null);
  const { historico, salvar, limpar } = useHistoricoConsultas("agregados-basica");
  ...
  const handleBuscar = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/apibrasil/agregados-basica?placa=${clean}`);
  ...
  const handleExemplo = useCallback(async () => {
    ...
    const { url } = await gerarUrlRelatorio("agregados-basica", placaSimulada, "PLACA", mockData);
        ...
        <div id="agregados-resultado" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <div style={{
            ...
            <div style={{ display: "flex", flexDirection: "column", flex: 1, minWidth: 0 }}>
              <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: COR_ACCENT, letterSpacing: "0.14em", fontWeight: 700 }}>AGREGADOS BÁSICA · {placa.toUpperCase()}</span>
