...
function formatarChassi(valor: string): string {
  return valor.toUpperCase().replace(/[^A-HJ-NPR-Z0-9]/g, "").slice(0, 17);
}
...
export function BuscaAgregadosChassiPanel() {
  const [chassi, setChassi] = useState("");
  const [loading, setLoading] = useState(false);
  const [erro, setErro] = useState<string | null>(null);
  const [resultado, setResultado] = useState<VeiculoResult | null>(null);
  const { historico, salvar, limpar } = useHistoricoConsultas("agregados-chassi");
  const [isFromHistory, setIsFromHistory] = useState(false);
  const [showHistoryMobile, setShowHistoryMobile] = useState(false);

  const handleChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    setChassi(formatarChassi(e.target.value));
    setErro(null);
  }, []);
  ...
  const handleBuscar = useCallback(async () => {
    ...
    if (clean.length !== 17) {
      setErro("Chassi inválido. Deve ter 17 caracteres alfanuméricos (sem I, O, Q).");
      return;
    }
    ...
    try {
      const res = await fetch(`/api/apibrasil/agregados-chassi?chassi=${clean}`);
  ...
  return (
    <div>
      ...
      <div className="search-bar-wrapper" style={{
        ...
        <div className="search-bar-container" style={{ display: "flex", gap: 12, alignItems: "stretch" }}>
          <div style={{ flex: 1, position: "relative" }}>
            ...
            <input
              id="chassi-decode-input"
              type="text"
              autoComplete="off"
              value={chassi}
              onChange={handleChange}
              onKeyDown={handleKeyDown}
              placeholder="9BFZF55A0E8008912"
              maxLength={17}
              style={{
                width: "100%",
      ...
      {/* ── Resultados ── */}
      {resultado && (
        <div id="chassi-resultado" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {/* Header */}
          <div style={{
            ...
            <div style={{ display: "flex", flexDirection: "column", flex: 1, minWidth: 0 }}>
              <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: COR_ACCENT, letterSpacing: "0.14em", fontWeight: 700 }}>DECODIFICADOR CHASSI · {chassi.toUpperCase()}</span>
              ...
              <span style={{
                ...
                CHASSI DECODIFICADO COM SUCESSO
              </span>
            </div>
            <button
            ...
            <Bloco titulo="Ficha Cadastral do Veículo" badge="INFO">
              <div className="snc-data-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 32px" }}>
                <div>
                  ...
                  <DataRow label="Placa" value={v.placa ? String(v.placa) : "—"} mono />
                  <DataRow label="Município / UF" value={v.municipio && v.uf ? `${v.municipio} / ${v.uf}` : "—"} />
                </div>
                <div>
                  <DataRow label="Espécie" value={String(v.especie ?? "—")} />
                  <DataRow label="Tipo" value={String(v.tipo ?? "—")} />
                  <DataRow label="Carroceria" value={String(v.carroceria ?? "—")} />
                  <DataRow label="Potência" value={String(v.potencia ?? "—")} />
                  <DataRow label="Cilindrada" value={String(v.cilindrada ?? "—")} mono />
                  <DataRow label="Capacidade Passageiros" value={String(v.capacidade_passageiros ?? "—")} mono />
                  <DataRow label="Procedência" value={String(v.procedencia ?? "—")} />
                </div>
              </div>
            </Bloco>
      ...
      <div className={`historico-wrapper ${showHistoryMobile ? "show-mobile" : ""}`}>
        ...
        <HistoricoConsultas
          ...
          onLimpar={limpar}
          corAccent="#D4A843"
          scrollTargetId="chassi-resultado"
        />
      </div>
      ...
      <style>{`
        ...
        @media (max-width: 768px) {
          ...
          #chassi-decode-input {
            ...
            padding: 16px 16px 16px 82px !important;
            letter-spacing: 0.08em !important;
          }
          #chassi-consultar-btn {
            width: 100% !important;
