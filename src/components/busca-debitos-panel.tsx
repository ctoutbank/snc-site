...
export function BuscaDebitosPanel() {
  ...
  const handleExemplo = useCallback((cenario: "clean" | "restricted") => {
    ...
    const { url } = gerarUrlRelatorio("debitos", placaSimulada, "PLACA", mockData);
      ...
      {r && (
        <div id="debitos-resultado" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <div style={{
            ...
            <button
              onClick={() => {
                const { url } = gerarUrlRelatorio("debitos", placa.replace(/[^A-Z0-9]/g, ""), "PLACA", r as unknown as Record<string, unknown>);
                window.open(url, "_blank");
              }}
              style={{
