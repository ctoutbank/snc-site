...
import { consultarAgregadosBasica, APIBrasilError } from "@/lib/apibrasil";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const raw = await consultarAgregadosBasica(placa);
    ...
    if (err instanceof APIBrasilError) {
      console.error(`[/api/apibrasil/agregados-basica] ${err.message}`, err.details);
    ...
    console.error("[/api/apibrasil/agregados-basica]", message);
