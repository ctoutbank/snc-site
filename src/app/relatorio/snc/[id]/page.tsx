...
import { deserializarDados, gerarProtocolo, DATASET_META } from "@/lib/relatorio";
...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  const payload = d ? deserializarDados(d) : null;
  const meta = payload ? DATASET_META[payload.dataset] : null;
  return {
    title: meta
      ? `SNC · ${meta.titulo} · Protocolo ${gerarProtocolo(id)}`
      : `SNC · Relatório ${id}`,
    description: meta?.subtitulo ?? "Relatório oficial SNC — Sistema Nacional de Conformidade.",
  };
}
...
export default async function RelatorioPage({ params, searchParams }: Props) {
  ...
  // ─── Renderização dos blocos de dados por dataset ─────────────────────────
    ...
    <>
      ...
      <script dangerouslySetInnerHTML={{ __html: `
        (function(){
          document.addEventListener('click',function(e){
            ...
            if(action==='back'){if(window.history.length>1)window.history.back();else window.location.href='/';}
      ...
      <div className="r-page">
        ...
        <div className="r-ms">
          ...
          <div><div className="l">Dataset</div><div className="v">{meta?.titulo?.split("—")[0]?.trim() ?? "SNC"}</div></div>
        ...
        <section className="r-sec">
          <div className="r-sh">
            ...
            <span className="badge">{payload?.dataset?.toUpperCase() ?? "DADOS"}</span>
          </div>
          <div className="r-summary">
            <div className="r-sl">
              ...
              <div className="pfs">
                <div>
                  <div className="l">Dataset</div>
                  <div className="v">{payload?.dataset ?? "—"}</div>
                </div>
                <div>
                  <div className="l">Emitido em</div>
                  <div className="v">{emitidoEm}</div>
                </div>
                <div>
                  <div className="l">Fonte</div>
                  <div className="v">{meta?.fonte ?? "APIBrasil"}</div>
                </div>
                <div>
                  <div className="l">Protocolo</div>
                  <div className="v">{protocolo}</div>
                </div>
              </div>
            </div>
            <div className="r-vrd">
        ...
        <section className="r-sec r-sec-results">
          ...
          {!payload ? (
            <div className="ds-block">
              <div className="ds-hd"><span>DADOS INDISPONÍVEIS</span></div>
              <div className="ds-row"><div className="ds-row-inner"><div className="dk">Erro</div><div className="dv">Não foi possível decodificar os dados do relatório. O link pode estar corrompido.</div></div></div>
            </div>
          ) : (
            <DadosDataset dataset={payload.dataset} resultado={resultado} />
          )}
...
// ─── Renderizador de dados por dataset ────────────────────────────────────────
...
function DRow({ label, value, chip }: { label: string; value?: unknown; chip?: { text: string; type?: "green" | "brass" | "red" } }) {
  ...
  return (
    <>
      <div className="ds-block">
        ...
        <DRow label="Placa" value={v.placa} />
        <DRow label="Marca" value={v.marca} />
        <DRow label="Modelo" value={v.modelo} />
        <DRow label="Ano Fabricação" value={v.anoFabricacao} />
        <DRow label="Ano Modelo" value={v.anoModelo} />
        <DRow label="Cor" value={v.cor} />
        <DRow label="Combustível" value={v.combustivel} />
        <DRow label="Categoria" value={v.categoria} />
...
function DadosProprietario({ r }: { r: Record<string, unknown> }) {
  ...
  return (
    <div className="ds-block">
      ...
      <DRow label="Nome" value={p.nome} />
      <DRow label="Documento (CPF/CNPJ)" value={p.documento} />
      <DRow label="Município / UF" value={p.municipio ? `${p.municipio} / ${p.uf ?? ""}` : "—"} />
      <DRow label="Marca / Modelo" value={p.marcaModelo} />
      <DRow label="Placa" value={p.placa} />
      <DRow label="RENAVAM" value={p.renavam} />
      <DRow label="Ano Fabricação" value={p.anoFabricacao} />
      <DRow label="Ano Modelo" value={p.anoModelo} />
      <DRow label="Cor" value={p.cor} />
      <DRow label="Combustível" value={p.combustivel} />
      <DRow label="Chassi" value={p.chassi} />
